...
ExerciseView::ExerciseView(QWidget* parent) : QGraphicsView(parent), boxColors(16, QColor("#007ea7"))
{
    ...
    // celebration colors
    colorVector.push_back(QColor("#2ec4b6"));
    colorVector.push_back(QColor("#e71d36"));
    colorVector.push_back(QColor("#ff9f1c"));
}
...
void ExerciseView::mouseMoveEvent(QMouseEvent *event)
{
            ...
            if (mode == Mode::SWAP)
            {
                ...
                for (int i = 0; i < itemList.size(); i++)
                {
                    if (pointerInBarBoxes(itemList.at(i)) && itemList.at(i) != activeRect)
                    {
                        ...
                        highlightRect->setBrush(QColor("#003459"));
                        dragging = true;
                        break;
                    }
                    else
    ...
    if (mode == MERGE)
    {
        ...
        for (int i = 0; i < itemList.size(); i++)
        {
            if (pointerInSubArray(itemList.at(i)))
            {
                QGraphicsRectItem* highlightRect = dynamic_cast<QGraphicsRectItem*>(itemList.at(i));
                highlightRect->setBrush(QColor("#ff1654"));
...
void ExerciseView::mousePressEvent(QMouseEvent *event)
{
    ...
    if (mode != MERGE)
    {
        for (int i = 0; i < itemList.size(); i++)
        {
            if (pointerInBarBoxes(itemList.at(i)))
            {
                ...
                activeRect->setZValue(1);
                barNumTexts[getGrooveIndexFromBox(activeRect)]->setZValue(1);
                dragging = true;
                break;
            }
        }
    ...
    {
        QGraphicsRectItem* mergingRect = nullptr;
        for (int i = 0; i < itemList.size(); i++)
        {
            if (pointerInSubArray(itemList.at(i)))
            {
                mergingRect = dynamic_cast<QGraphicsRectItem*>(itemList.at(i));
                break;
            }
        }

        if(mergingRect != nullptr)
        {
            int oldGroove = getGrooveIndexFromBox(mergingRect);
            int xPos = getXPosFromGroove(emptyGrooveLocations.front());
            emptyGrooveLocations.pop_front();
            int yPos = getBoxYPos(mergingRect);
            mergingRect->setPos(xPos, yPos);
            barNumTexts[oldGroove]->setPos(xPos, yPos-20);
            for(int i = 0; i < subArray.size(); i++)
            {
                if (subArray[i] == mergingRect)
                {
                    subArray[i] = nullptr;
                    break;
                }
            ...
            if (emptyGrooveLocations.size() == 0)
            {
                resetBoxesFromView();
...
{   
    if (dragging == true && activeRect != nullptr)
    {
        ...
        else if (mode == Mode::SHIFT)
        {
            ...
            if (passingGroove != activeGroove)
            {
                (barBoxes[passingGroove], activeGroove);
            }
            QPointF resetPoint(getXPosFromGroove(getGrooveIndexFromBox(activeRect)), getBoxYPos(activeRect));
            activeRect->setPos(resetPoint);
            barNumTexts[getGrooveIndexFromBox(activeRect)]->setPos(resetPoint.rx(), resetPoint.ry()-20);
    ...
    for(int i = 0; i < 16; i++)
    {
        barBoxes[i]->setZValue(0);
        barNumTexts[i]->setZValue(0);
    }
    QGraphicsView::mouseReleaseEvent(event);
}

// Updates model with array in view.
void ExerciseView::resetBoxesFromView()
{
    std::vector<QGraphicsRectItem *> barBoxesList(16, nullptr);
    std::vector<QGraphicsTextItem* > barTextsList(16, nullptr);

    for (int i = 0; i < 16; i++)
    {
        int xPos = barBoxes[i]->pos().rx()+10;
        int groove = getGrooveIndexAt(xPos);
        barBoxesList[groove] = barBoxes[i];
        barTextsList[groove] = barNumTexts[i];
    }
    for(int i = 0; i < 16; i++)
    {
        barBoxes[i] = barBoxesList[i];
        barNumTexts[i] = barTextsList[i];
    }
}
...
void ExerciseView::construct_boxes(int box_values[], bool random)
{
    ...
    ground = scene->addRect(50, 660,1000,10,blackpen, Qt::black);
...
// Return the Y coordinate position for any box for the merge position
int ExerciseView::getBoxYPosMerge(const QGraphicsRectItem *box)
{
    return static_cast<int>(floor(300 - box->rect().height()));
}
...
bool ExerciseView::pointerInSubArray(void *pointer)
{
    for (int i = 0; i < subArray.size(); i++)
        if (pointer == subArray[i])
            return true;
    return false;
}
...
void ExerciseView::startAlgorithm(ExerciseView::Mode algoMode)
{
    ...
    if (timer)
    {
        emit disableButtons();
        timer->start(4);
        setupMergeSortPositions();
    }
    else
        emit motionComplete();
    if (mode == MERGE)
    {
        higherGround->setVisible(true);
        dividingLine->setVisible(true);
        for(int i = 0; i < 16; i++)
        {
            barBoxes[i]->setFlag(QGraphicsRectItem::ItemIsMovable, false);
        }
    }
    else
    {
...
void ExerciseView::getNextMergeBoxes()
{
    emptyGrooveLocations.clear();
    if(mergeLocation >= 16)
    {
        mergeLocation = 0;
        mergeStep *= 2;
    ...
    int j = 0;
    for(int i = mergeLocation; i < (mergeLocation + 2*mergeStep); i++)
    {
        // Set Location of Merge Boxes
        int newY1 = getBoxYPosMerge(barBoxes[i]);
        int newX1 = getXPosFromGroove(8 - mergeStep + j);
        barBoxes[i]->setPos(newX1,newY1);
        barNumTexts[i]->setPos(newX1,newY1-20);
        emptyGrooveLocations.push_back(i);
        subArray.push_back(barBoxes[i]);
        j++;
    }
}
void ExerciseView::setupMergeSortPositions()
{
    QBrush whiteBrush(Qt::white);
    QPen blackpen(Qt::black);
    blackpen.setWidth(1);

    higherGround = scene->addRect(50, 300,1000,1,blackpen,whiteBrush);
    dividingLine = scene->addRect(546, -200,1,500,blackpen,whiteBrush);
}

void ExerciseView::incrementMergeLocation()
{
    mergeLocation += mergeStep*2;
    getNextMergeBoxes();
}

void ExerciseView::resetMergeVars()
{
    mergeStep = 1;
    mergeLocation = 0;
}

void ExerciseView::resetBoxColors()
{
    for(int i = 0; i < 16; i++)
    {
        boxColors[i] =  QColor("#007ea7");
        barBoxes[i]->setBrush(boxColors[i]);
    }
}
