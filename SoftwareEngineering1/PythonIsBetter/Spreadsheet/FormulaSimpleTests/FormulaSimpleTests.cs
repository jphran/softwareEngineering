...
namespace FormulaTestCases
{
    ...
    public class UnitTests
    {
        ...
        /// <summary>
        /// Test different valid expressions
        /// </summary>
        [TestMethod]
        public void Evaluate7()
        {
            Formula f1 = new Formula("x+4*5");
            Assert.AreEqual(f1.Evaluate(Lookup4), 24.0, 1e-6);

            Formula f2 = new Formula("x+4*5");
            Assert.AreEqual(f2.Evaluate(Lookup4), 24.0, 1e-6);

            Formula f3 = new Formula("(x+4)*5");
            Assert.AreEqual(f3.Evaluate(Lookup4), 40.0, 1e-6);

            Formula f4 = new Formula("x+3.2");
            Assert.AreEqual(f4.Evaluate(Lookup4), 7.2, 1e-6);

            Formula f5 = new Formula("x+4/5");
            Assert.AreEqual(f5.Evaluate(Lookup4), 4.8, 1e-6);

            Formula f6 = new Formula("x+(4/5)");
            Assert.AreEqual(f6.Evaluate(Lookup4), 4.8, 1e-6);
        }
